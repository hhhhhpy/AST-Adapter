...
class VideoClsDataset_sparse(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
        ...
        if args.clip_num is not None:
            self.dataset_samples = self.dataset_samples[:args.clip_num]
            self.label_array = self.label_array[:args.clip_num]
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            return buffer, int(self.label_array[index])-1 if self.args.data_set=='UCF101' else self.label_array[index], index, {}
