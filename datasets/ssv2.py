...
class SSVideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
        ...
        if args.clip_num is not None:
            self.dataset_samples = list(cleaned.values[:, 0])[:args.clip_num]
            self.label_array = list(cleaned.values[:, 1])[:args.clip_num]
            # tmp_arr = []
            # tmp_data = []
            # trans = {7:0,10:1,21:2,25:3,32:4,38:5}
            # for i,d in enumerate(self.label_array):
            #     if d in [34,35,36,37,38,39]:
            #         tmp_arr.append(self.label_array[i]-34)
            #         # tmp_arr.append(trans[d])
            #         tmp_data.append(self.dataset_samples[i])
           
            # self.label_array = tmp_arr
            # self.dataset_samples = tmp_data
        if (mode == 'train'):
            pass
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            chunk_nb, split_nb = self.test_seg[index]
            buffer = self.loadvideo_decord(sample)
            # buffer = self.loadvideo_decord_k400(sample,chunk_nb=chunk_nb)
    ...
    def loadvideo_decord(self, sample, sample_rate_scale=1):
        ...
        if self.mode == 'test':
            ...
            # print(len(vr))
            tick = (len(vr)-self.test_num_segment)/float(self.num_segment)
            all_index =[]
            for i in range(self.test_num_segment):
                all_index = all_index + [int(i + tick*x) for x in range(self.num_segment)]
            # print(all_index)
